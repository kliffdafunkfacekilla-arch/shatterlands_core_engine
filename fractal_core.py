...
def process_cluster_fidelity(engine_tick, global_hex_id, conn):
    ...
    avg_p1 = sum(hx.p1 for hx in micro_hexes.values()) // 61
    avg_p2 = sum(hx.p2 for hx in micro_hexes.values()) // 61
    avg_p3 = sum(hx.p3 for hx in micro_hexes.values()) // 61
    avg_res = sum(hx.res for hx in micro_hexes.values()) // 61
