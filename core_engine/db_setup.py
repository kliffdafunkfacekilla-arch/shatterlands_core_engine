...
def init_database(db_path=DB_PATH):
    ...
    CREATE TABLE IF NOT EXISTS settlements (
        ...
        spark_born_population INTEGER DEFAULT 25,
    ...
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_diplomacy_a ON diplomacy_relations (settlement_a_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_diplomacy_b ON diplomacy_relations (settlement_b_id)")
    ...
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_crimes_settlement ON crimes (settlement_id)")

    # Ensure foreign key enforcement for new tables
    ...
    CREATE TABLE IF NOT EXISTS farms (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        global_hex_id INTEGER NOT NULL,
        micro_q INTEGER NOT NULL,
        micro_r INTEGER NOT NULL,
        settlement_id INTEGER NOT NULL,
    ...
    cursor.execute("DROP TABLE IF EXISTS criminal_hideouts")
    cursor.execute('''
    ...
    CREATE TABLE IF NOT EXISTS paragons (
        ...
        goal TEXT NOT NULL,
        stat_mass INTEGER DEFAULT 1,
        stat_ordo INTEGER DEFAULT 1,
        stat_motus INTEGER DEFAULT 1,
        stat_flux INTEGER DEFAULT 1,
        stat_vita INTEGER DEFAULT 1,
        stat_nexus INTEGER DEFAULT 1,
        stat_ratio INTEGER DEFAULT 1,
        stat_anumis INTEGER DEFAULT 1,
        stat_lux INTEGER DEFAULT 1,
        stat_omen INTEGER DEFAULT 1,
        stat_aura INTEGER DEFAULT 1,
        stat_lex INTEGER DEFAULT 1,
        blessing TEXT,
        flaw TEXT,
        quirk1 TEXT,
        quirk2 TEXT,
    ...
    CREATE TABLE IF NOT EXISTS chaos_agents (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        type TEXT, -- 'Criminal', 'Cultist', 'Warden'
        global_hex_id INTEGER NOT NULL,
        micro_q INTEGER NOT NULL,
        micro_r INTEGER NOT NULL,
...
def seed_planet(db_path=DB_PATH, R=57, config=None):
    ...
    hex_coords = []
    L = 14  # Scaled down L so the map has ~12,500 hexes (comparable to radius 57)

    def add_up_tri(q0, r0):
        for r in range(r0, r0 + L):
            for q in range(q0 - (r - r0), q0 + 1):
                hex_coords.append((q, r))

    def add_dn_tri(q0, r0):
        for r in range(r0 - L + 1, r0 + 1):
            for q in range(q0, q0 + (r0 - r) + 1):
                hex_coords.append((q, r))

    # Middle Strip
    for i in range(5):
        add_up_tri(i*L + i, 0)
        add_dn_tri(i*L + i + 1, L-1)
    # Top Row
    for i in range(5):
        add_up_tri((i+1)*L + i, -L)
    # Bottom Row
    for i in range(5):
        add_dn_tri(i*L + i + 1 - L, 2*L - 1)

    hex_coords = list(set(hex_coords)) # Remove duplicates at edges

    prisons = [
        (L, -L),
        (0,0), (L+1,0), (2*L+2,0), (3*L+3,0), (4*L+4,0),
        (1, L-1), (L+2, L-1), (2*L+3, L-1), (3*L+4, L-1), (4*L+5, L-1),
        (-L+1, 2*L-1)
    ]

    # Discard old leyline logic since it was center-focused
    # The leylines are now visually drawn in app.js using the D20 net edges

    # Initialize basic properties
    leyline_paths = {}
    elevation = {k: 0.0 for k in hex_coords}
    river_volume = {k: 0 for k in hex_coords}
    ...
    for q, r in hex_coords:
        lon_normalized = (q - (-L)) / (5 * L)  # roughly 0 to 1
        lat_normalized = (r - (-L)) / (3 * L)  # 0 to 1
        lon = lon_normalized * math.pi * 2 - math.pi
        lat = lat_normalized * math.pi - (math.pi / 2)
    ...
    for p_q, p_r in prisons:
        cursor.execute("SELECT id FROM global_hexes WHERE q=? AND r=?", (p_q, p_r))
        p_row = cursor.fetchone()
        if p_row:
            cursor.execute("INSERT INTO world_entities (type, global_hex_id, alignment) VALUES (?, ?, ?)", ("Prison", p_row[0], "Chaos"))
    ...
    for idx, (q, r) in enumerate(hex_coords):
        ...
        elevation_val = max(0, min(15, int(abs(elevation_map[(q, r)]) * 15)))
        pack_geo = (biome & 0xF) | ((elevation_val & 0xF) << 4)
    ...
    cursor.execute("INSERT INTO factions (name, treasury, technology_level) VALUES ('Kingdom of Arcanum', 5000.0, 3)")
    cursor.execute("INSERT INTO factions (name, treasury, technology_level) VALUES ('Free Cities', 3000.0, 2)")
    cursor.execute("SELECT id FROM factions WHERE name='Kingdom of Arcanum'")
    faction_a_id = cursor.fetchone()[0]
    cursor.execute("SELECT id FROM factions WHERE name='Free Cities'")
    faction_b_id = cursor.fetchone()[0]
    # Insert a few settlements on random non‑chaos hexes
    cursor.execute("SELECT id FROM global_hexes WHERE chaos_domain IS NULL ORDER BY RANDOM() LIMIT 6")
    hex_ids = [row[0] for row in cursor.fetchall()]
    settlement_names = ['Eldermere', 'Rivenshire', 'Stonewatch', 'Goldhaven', 'Mistmoor', 'Highvale']
    for i, hex_id in enumerate(hex_ids):
        name = settlement_names[i]
        faction_id = faction_a_id if i % 2 == 0 else faction_b_id
        cursor.execute(
            "INSERT INTO settlements (faction_id, global_hex_id, name, settlement_level, population, wealth, security_points) VALUES (?,?,?,?,100,500.0,10.0)",
            (faction_id, hex_id, name, 1)
        )
    # Seed one starting paragon per faction
    cursor.execute("SELECT id FROM settlements WHERE faction_id=? LIMIT 1", (faction_a_id,))
    s_a = cursor.fetchone()
    cursor.execute("SELECT id FROM settlements WHERE faction_id=? LIMIT 1", (faction_b_id,))
    s_b = cursor.fetchone()
    if s_a:
        cursor.execute(
            "INSERT INTO paragons (settlement_id, name, descriptor, goal) VALUES (?,?,?,?)",
            (s_a[0], 'Aldric the Peacekeeper', 'Diplomat', 'Foster peace between factions')
        )
    if s_b:
        cursor.execute(
            "INSERT INTO paragons (settlement_id, name, descriptor, goal) VALUES (?,?,?,?)",
            (s_b[0], 'Maren Ironblade', 'Warrior', 'Defend the Free Cities at all costs')
        )
    conn.commit()
    conn.close()
