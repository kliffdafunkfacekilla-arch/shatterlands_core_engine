...
class GlobalEngine:
    ...
    def log_event(self, category, msg, conn):
        cursor = conn.cursor()
        cursor.execute("INSERT INTO event_log (tick, category, message) VALUES (?, ?, ?)", (self.tick, category, msg))
    ...
    def apply_chaos_event(self, domain, name, pop, sec, inventory, q, r, p_geo, cursor, conn):
        ...
        if domain == "Mass":
            if is_effect_a: # Crushing
                ...
                self.log_event("Chaos", f"Mass crushed {name}! Wood destroyed.", conn)
            else: # Anti-Gravity
                float_pop = int(pop * 0.2)
                pop -= float_pop
                inventory.setdefault("Survival", {})["Base Food"] = 0
                self.log_event("Chaos", f"Anti-gravity in {name} flung {float_pop} people into the sky!", conn)
                
        elif domain == "Ordo":
            if is_effect_a: # Freezing
                local_farm = 0.0
                pop -= int(pop * 0.1)
                self.log_event("Chaos", f"Absolute zero struck {name}, freezing ecology and killing citizens.", conn)
            else: # Structuring
                inventory["Tags"] = inventory.get("Tags", []) + ["Rigid"]
                self.log_event("Chaos", f"Ordo structured {name}, removing brittle tags.", conn)
                
        elif domain == "Motus":
            if is_effect_a: # Sonic Boom
                inventory.setdefault("Building", {})["Stone"] = 0
                sec = max(0, sec - 20)
                self.log_event("Chaos", f"Sonic boom shattered stone and security in {name}!", conn)
            else: # Frictionless
                inventory["Building"] = {}
                self.log_event("Chaos", f"Resources in {name} slid away frictionlessly across the map!", conn)
            ...
            if is_effect_a: # Transmutation
                ...
                inventory.setdefault("Building", {})["Wood"] = 0
                self.log_event("Chaos", f"Flux transmuted wood to food in {name}!", conn)
            else: # Terrain Shift
                ...
                if row:
                    ...
                    p_geo = n_geo
                self.log_event("Chaos", f"Flux swapped the geography of {name} with a neighboring hex!", conn)
                
        elif domain == "Vita":
            if is_effect_a: # Fecundity
                local_farm *= 5.0
                pop -= int(pop * 0.1)
                self.log_event("Chaos", f"Hyper-evolution in {name} exploded crop yields but caused lethal cancers!", conn)
                ...
                self.log_event("Chaos", f"Toxic overgrowth permanently mutated {name} into a deadly jungle!", conn)
                
        elif domain == "Nexus":
            if is_effect_a: # Fire Storms
                inventory.setdefault("Building", {})["Wood"] = 0
                self.log_event("Chaos", f"Fire storms burned all the wood in {name}!", conn)
            else: # Violent Reactions
                if inventory.get("Reagents", {}).get("Oils", 0) > 0:
                    inventory["Reagents"]["Oils"] = 0
                    sec = max(0, sec - 50)
                    self.log_event("Chaos", f"Volatile oils exploded violently in {name}!", conn)
                    
        elif domain == "Ratio":
            if is_effect_a: # Logic Break
                inventory["Tags"] = inventory.get("Tags", []) + ["Logic Broken"]
                self.log_event("Chaos", f"Logic broke in {name}, randomizing construction costs!", conn)
            else: # Logic Riots
                sec = max(0, sec - 30)
                self.log_event("Chaos", f"People logiced themselves into violence in {name}, triggering massive riots!", conn)
                
        elif domain == "Anumis":
            if is_effect_a: # Tag Inversion
                inventory["Tags"] = inventory.get("Tags", []) + ["Inverted Physics"]
                self.log_event("Chaos", f"Anumis inverted physical properties in {name}!", conn)
            else: # Whirlwind
                inventory.setdefault("Building", {})["Stone"] = 0
                inventory.setdefault("Survival", {})["Base Food"] = 0
                self.log_event("Chaos", f"An arcane whirlwind shredded resources in {name}!", conn)
                
        elif domain == "Lux":
            if is_effect_a: # Intense Light
                local_farm *= 0.5
                pop -= int(pop * 0.05)
                self.log_event("Chaos", f"Blinding light scorched {name}, causing sunburns and blindness!", conn)
            else: # Illusions
                sec = 0
                self.log_event("Chaos", f"Terrifying illusions tanked morale and security in {name}!", conn)
                
        elif domain == "Omen":
            if is_effect_a: # Fast Forward
                inventory.setdefault("Survival", {})["Base Food"] = inventory.get("Survival", {}).get("Base Food", 0) + (pop * 5)
                self.log_event("Chaos", f"Omen simulated ticks ahead for {name}, generating instant food!", conn)
            else: # Rot
                inventory.setdefault("Survival", {})["Base Food"] = 0
                pop -= int(pop * 0.1)
                self.log_event("Chaos", f"Temporal rot instantly decayed food and aged citizens in {name}!", conn)
                
        elif domain == "Aura":
            if is_effect_a: # Euphoria
                sec = 100
                local_farm = 0.0
                self.log_event("Chaos", f"Toxic euphoria paralyzed {name} in bliss. 0 production.", conn)
            else: # Hate Riots
                sec = 0
                pop -= int(pop * 0.2)
                self.log_event("Chaos", f"Bloody hate riots decimated {name}!", conn)
            ...
            if is_effect_a: # Stasis
                ...
                local_farm = 0.0
                local_consume = 0.0
                self.log_event("Chaos", f"Lex locked {name} in an absolute void of stasis!", conn)
            else: # Holy War
                local_farm = 0.0
                pop -= int(pop * 0.1)
                self.log_event("Chaos", f"A violent theological war halted production in {name} with massive fatalities!", conn)
    ...
    def process_world_entities(self, cursor, conn):
        ...
        cursor.execute("SELECT id, type, global_hex_id, radius, duration, alignment FROM world_entities")
        ...
        for e_id, e_type, g_id, e_rad, e_dur, e_align in entities:
            e_dur -= 1
            if e_dur <= 0:
            ...
            if new_g_id_row:
                ...
                cursor.execute("UPDATE world_entities SET global_hex_id=?, duration=?, type=?, alignment=? WHERE id=?", (new_g_id, e_dur, e_type, e_align, e_id))
                alive_entities.append((e_id, e_type, new_g_id, e_rad, nq, nr, e_align))
            else:
                cursor.execute("DELETE FROM world_entities WHERE id=?", (e_id,))
    ...
    def trigger_tick(self):
        ...
        if warden_spire:
            s_id, f_id, g_hex_id, name, s_level, pop, wealth, sec, inv_str, hidden, loadout_str, q, r, p_geo, p_meso, p_eco, micro, rvol, islake, domain, f_rule = warden_spire
        ...
        for row in settlements:
            s_id, f_id, g_hex_id, name, s_level, pop, wealth, sec, inv_str, hidden, loadout_str, q, r, p_geo, p_meso, p_eco, micro_json, river_vol, is_lake, domain, f_rule = row
            ...
            reachable_hexes = [hx for hx in micro_hexes.values() if 0 < hx.distance_from_center <= s_level]
